...
if (args.Length < 4) throw new ArgumentException("usage: PakExtract PAK OUTPUT LOWER UPPER");
var pakPath = args[0];
var outputPath = args[1];
var targetLower = uint.Parse(args[2]);
var targetUpper = uint.Parse(args[3]);
...
var entryBytes = reader.ReadBytes(checked(totalFiles * 48));
...
if (offset == 0 || compressedSize == 0) throw new FileNotFoundException("Target entry not found");
...
if (data.LongLength != decompressedSize) throw new InvalidDataException($"Size mismatch: {data.Length} != {decompressedSize}");
